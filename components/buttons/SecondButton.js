...
function SecondButton({ children, color, hover, isDisable, type, typeBtn, size, href, hasLeftIcon, hasRightIcon, icon, shadow, radius, full, textColor, bgHover, textSize, onClick }) {
  if (href) {
    ...
    switch (typeBtn) {
      case "outline":
        return <button
          className={`py-2 hover-animation px-4 flex justify-center items-center rounded-xl border ${full ? "w-full" : "w-auto"} ${color} ${textColor} hover:text-white ${bgHover} ${textSize}`}
          type={type}
          disabled={isDisable}
          onClick={onClick}
        >
      ...
      case "filled":
        return <button
          className={`px-6 py-3 hover-animation text-white flex justify-center items-center ${full ? "w-full" : "w-auto"} ${shadow} ${color} ${radius} ${textColor} ${textSize}`}
          type={type}
          disabled={isDisable}
          onClick={onClick}
        >
